...
// CHECK-BASIC-LABEL: === Variable Statistics for Translation Unit ===
...
// CHECK-STATIC-LABEL: === Variable Statistics for Translation Unit ===
...
// CHECK-PARAMS-LABEL: === Variable Statistics for Translation Unit ===
...
// CHECK-ALL-LABEL: === Variable Statistics for Translation Unit ===
...
// CHECK-EDGE-LABEL: === Variable Statistics for Translation Unit ===
...
// Статическая глобальная структура
...
void test_edge(int param) {
    ...
    // Статическая локальная переменная-указатель
    static int* static_ptr = 0;
    
    // Присваивания чтобы компилятор не ругался
    local_array[0] = param;
